...
public class SymbolInfoTest extends TestCase {
  ...
  public void testSymbolInfo() throws Exception {
    ...
    info = DataMatrixSymbolInfo.lookup(35, SymbolShapeHint.FORCE_NONE, fixedSize, fixedSize, false);
    assertEquals(26, info.getSymbolWidth());
    assertEquals(26, info.getSymbolHeight());

    info = DataMatrixSymbolInfo.lookup(45, SymbolShapeHint.FORCE_NONE, fixedSize, fixedSize, false);
    assertNull(info);

    Dimension minSize = fixedSize;
    Dimension maxSize = new Dimension(32, 32);

    info = DataMatrixSymbolInfo.lookup(35, SymbolShapeHint.FORCE_NONE, minSize, maxSize, false);
    assertEquals(26, info.getSymbolWidth());
    assertEquals(26, info.getSymbolHeight());

    info = DataMatrixSymbolInfo.lookup(40, SymbolShapeHint.FORCE_NONE, minSize, maxSize, false);
    assertEquals(26, info.getSymbolWidth());
    assertEquals(26, info.getSymbolHeight());

    info = DataMatrixSymbolInfo.lookup(45, SymbolShapeHint.FORCE_NONE, minSize, maxSize, false);
    assertEquals(32, info.getSymbolWidth());
    assertEquals(32, info.getSymbolHeight());

    info = DataMatrixSymbolInfo.lookup(63, SymbolShapeHint.FORCE_NONE, minSize, maxSize, false);
    assertNull(info);
  }
