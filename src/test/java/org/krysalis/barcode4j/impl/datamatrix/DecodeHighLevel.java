...
public class DecodeHighLevel {
  ...
  public static void decode(String codewords) {
    ...
    while (idx < len) {
      ...
      switch (ch) {
        ...
        default:
          if (ch >= 1 && ch <= 128) {
            System.out.println("char: " + (char) (ch - 1) + " - " + (ch - 1) + " (" + (int) ch + ")");
  ...
  private static int decodeBase256(String codewords, int start) {
    ...
    for (int i = 0; i < sectionLen; i++) {
      ...
      System.out.println("byte: " + urcw + " - " + (int) urcw + " (0x" + Integer.toHexString(urcw) + ")" + ", org: " + cw + " - " + (int) cw + " (0x" + Integer.toHexString(cw) + ")");
    }
    return idx;
  }
