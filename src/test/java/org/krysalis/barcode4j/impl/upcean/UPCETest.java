...
public class UPCETest extends TestCase {
  ...
  private static final String[][] COMPACTION_TESTS = {{"01278907", "012000007897"}, {"01278916", "012100007896"}, {"01278925", "012200007895"}, {"01238935", "012300000895"}, {"01248934", "012400000894"}, {"01258933", "012500000893"}, {"01268932", "012600000892"}, {"01278931", "012700000891"}, {"01288930", "012800000890"}, {"01298939", "012900000899"}, {"01291944", "012910000094"}, {"01291155", "012911000055"}, {"01291162", "012911000062"}, {"01291179", "012911000079"},
      {"01291186", "012911000086"}, {"00102546", "001020000056"}, {"01234133", "012300000413"}};

  public void testMessageCompaction() throws Exception {
    for (int i = 0; i < COMPACTION_TESTS.length; i++) {
      assertEquals(COMPACTION_TESTS[i][1] + " must be compacted to " + COMPACTION_TESTS[i][0], COMPACTION_TESTS[i][0], UPCELogicImpl.compactMessage(COMPACTION_TESTS[i][1]));
      String nocheck = COMPACTION_TESTS[i][1].substring(0, 11);
      assertEquals(nocheck + " must be compacted to " + COMPACTION_TESTS[i][0], COMPACTION_TESTS[i][0], UPCELogicImpl.compactMessage(nocheck));
    }
  ...
  public void testMessageExpansion() throws Exception {
    for (int i = 0; i < COMPACTION_TESTS.length; i++) {
      assertEquals(COMPACTION_TESTS[i][0] + " must be expanded to " + COMPACTION_TESTS[i][1], COMPACTION_TESTS[i][1], UPCELogicImpl.expandMessage(COMPACTION_TESTS[i][0]));
      String nocheck = COMPACTION_TESTS[i][0].substring(0, 7);
      assertEquals(nocheck + " must be expanded to " + COMPACTION_TESTS[i][1], COMPACTION_TESTS[i][1], UPCELogicImpl.expandMessage(nocheck));
    }
  }
  ...
  public void testLogic() throws Exception {
    ...
    expected = "<BC>" + "<SBG:upc-ean-guard:null>B1W1B1</SBG>" + "<SBG:upc-ean-lead:0>" + "</SBG>" + "<SBG:upc-ean-group:425261>" + "<SBG:msg-char:4>W2B3W1B1</SBG>" + "<SBG:msg-char:2>W2B1W2B2</SBG>" + "<SBG:msg-char:5>W1B3W2B1</SBG>" + "<SBG:msg-char:2>W2B2W1B2</SBG>" + "<SBG:msg-char:6>W1B1W1B4</SBG>" + "<SBG:msg-char:1>W2B2W2B1</SBG>" + "</SBG>" + "<SBG:upc-ean-check:4>" + "</SBG>" + "<SBG:upc-ean-guard:null>W1B1W1B1W1B1</SBG>" + "</BC>";
