...
public class URLUtil {
  ...
  private static byte[] parseDataURL(String url, String encoding) throws IOException {
    ...
    if (header.endsWith(";base64")) {
      Base64InputStream in = new Base64InputStream(new java.io.StringReader(data));
      ByteArrayOutputStream baout = new ByteArrayOutputStream();
      IOUtil.copy(in, baout);
      IOUtil.closeQuietly(in);
      return baout.toByteArray();
  ...
  public static String getURL(String message) {
    if (URLUtil.isURL(message)) {
      String url = message.substring(URLUtil.URL_START.length(), message.length() - URLUtil.URL_END.length());
      return url;
    } else {
      return null;
    }
