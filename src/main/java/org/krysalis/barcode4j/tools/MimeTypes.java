...
public class MimeTypes {
  ...
  private static final String[][] FORMAT_MAPPINGS = {{"svg", MIME_SVG}, {"eps", MIME_EPS}, {"image/eps", MIME_EPS}, {"tif", MIME_TIFF}, {"tiff", MIME_TIFF}, {"jpg", MIME_JPEG}, {"jpeg", MIME_JPEG}, {"png", MIME_PNG}, {"image/png", MIME_PNG}, {"gif", MIME_GIF}, {"image/x-bmp", MIME_BMP}, {"bmp", MIME_BMP}};
  ...
  public static String expandFormat(String format) {
    ...
    for (int i = 0; i < FORMAT_MAPPINGS.length; i++) {
      if (format.equalsIgnoreCase(FORMAT_MAPPINGS[i][0]) || format.equals(FORMAT_MAPPINGS[i][1])) {
