...
public class DataMatrixHighLevelEncoder implements DataMatrixConstants {
  ...
  private static class EncoderContext {
    ...
    public EncoderContext(String msg) {
      ...
      for (int i = 0, c = msgBinary.length; i < c; i++) {
        ...
        if (ch == '?' && msg.charAt(i) != '?') {
          throw new IllegalArgumentException("Message contains characters outside " + DEFAULT_ASCII_ENCODING + " encoding.");
        }
        sb.append(ch);
      }
  ...
  private static class ASCIIEncoder implements Encoder {
    ...
    public void encode(EncoderContext context) {
      ...
      if (n >= 2) {
        context.writeCodeword(encodeASCIIDigits(context.msg.charAt(context.pos), context.msg.charAt(context.pos + 1)));
        context.pos += 2;
      } else {
  ...
  private static boolean isNativeX12(char ch) {
    return isX12TermSep(ch) || (ch == 32) // SPACE
