...
public class Interleaved2Of5LogicImpl {
  ...
  private static final byte[][] CHARSET = {{1, 1, 2, 2, 1}, {2, 1, 1, 1, 2}, {1, 2, 1, 1, 2}, {2, 2, 1, 1, 1}, {1, 1, 2, 1, 2}, {2, 1, 2, 1, 1}, {1, 2, 2, 1, 1}, {1, 1, 1, 2, 2}, {2, 1, 1, 2, 1}, {1, 2, 1, 2, 1}};
  ...
  protected String doHandleChecksum(StringBuffer sb, ChecksumMode mode) {
    if (mode == ChecksumMode.CP_ADD) {
      ...
      if (!validateChecksum(sb.toString())) {
        throw new IllegalArgumentException("Message '" + sb.toString() + "' has a bad checksum. Expected: " + calcChecksum(sb.substring(0, sb.length() - 1)));
      }
      if (displayChecksum) {
        return sb.toString();
      } else {
