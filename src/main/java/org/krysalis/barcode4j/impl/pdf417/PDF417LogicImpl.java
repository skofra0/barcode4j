...
public class PDF417LogicImpl {
  ...
  private static void encodeLowLevel(String fullCodewords, int c, int r, int errorCorrectionLevel, TwoDimBarcodeLogicHandler logic) {
    int idx = 0;
    for (int y = 0; y < r; y++) {
      int cluster = (y % 3);
      logic.startRow();
  ...
  public static Dimension determineDimensions(PDF417Bean pdf417Bean, int sourceCodeWords) {
    ...
    for (int cols = minCols; cols <= maxCols; cols++) {
      ...
      double newRatio = ((17 * cols + 69) * pdf417Bean.getModuleWidth()) / (rows * pdf417Bean.getBarHeight());
