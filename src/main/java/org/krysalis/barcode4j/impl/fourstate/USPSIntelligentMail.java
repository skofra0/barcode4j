...
public class USPSIntelligentMail extends ConfigurableBarcodeGenerator implements Configurable {
  ...
  public void configure(Configuration cfg) throws ConfigurationException {
    ...
    Length mw = new Length(cfg.getChild("module-width").getValue(USPSIntelligentMailBean.DEFAULT_MODULE_WIDTH_INCH + Length.INCH), Length.INCH);
    getUSPSIntelligentMailBean().setModuleWidth(mw.getValueAsMillimeter());

    super.configure(cfg);

    // Checksum mode
    getUSPSIntelligentMailBean().setChecksumMode(ChecksumMode.byName(cfg.getChild("checksum").getValue(ChecksumMode.CP_AUTO.getName())));

    // Inter-character gap width
    Length igw = new Length(cfg.getChild("interchar-gap-width").getValue(USPSIntelligentMailBean.DEFAULT_INTERCHAR_GAP_WIDTH_INCH + Length.INCH), Length.INCH);
    ...
    Length ah = new Length(cfg.getChild("ascender-height").getValue(USPSIntelligentMailBean.DEFAULT_ASCENDER_HEIGHT_INCH + Length.INCH), Length.INCH);
    getUSPSIntelligentMailBean().setAscenderHeight(ah.getValueAsMillimeter());

    Length th = new Length(cfg.getChild("track-height").getValue(USPSIntelligentMailBean.DEFAULT_TRACK_HEIGHT_INCH + Length.INCH), Length.INCH);
