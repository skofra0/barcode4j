...
public class EPSCanvasProvider extends AbstractCanvasProvider {
  ...
  private void writeHeader(double width, double height) throws IOException {
    ...
    writer.write("%%BoundingBox: 0 0 " + Math.round(Math.ceil(widthpt)) + " " + Math.round(Math.ceil(heightpt)) + "\n");
    writer.write("%%HiResBoundingBox: 0 0 " + format(widthpt) + " " + format(heightpt) + "\n");
  ...
  public void deviceText(String text, double x1, double x2, double y1, String fontName, double fontSize, TextAlignment textAlign) {
    ...
    try {
      ...
      if (textAlign == TextAlignment.TA_LEFT) {
        ...
        writer.write("(" + text + ") " + formatmm(x1) + " " + formatmm(x2) + " " + formatmm(this.height - y1) + " rt\n");
        ...
        writer.write("(" + text + ") " + formatmm(x1) + " " + formatmm(x2) + " " + formatmm(this.height - y1) + " jt\n");
      }
    } catch (IOException ioe) {
      firstError = ioe;
    }
